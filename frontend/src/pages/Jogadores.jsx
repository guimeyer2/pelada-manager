...
const inputClass =
  "bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500/20 transition-[border-color,box-shadow] duration-150";
  ...
  return (
    <div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-8">
        <p className="text-sm text-gray-400 mb-3 font-medium">{t("players.addSection")}</p>
        <div className="flex gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <label className="text-sm text-gray-400">{t("players.rating")}</label>
            <input
              type="number"
              min={1}
              max={10}
      ...
      <div className="flex flex-col gap-1.5">
        {players.map((player) => (
          <div
            key={player.id}
            className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 flex flex-col gap-2 hover:border-gray-700 transition-[border-color] duration-150"
          >
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="bg-gray-800 border border-gray-700 rounded-lg px-2 py-1 text-sm w-16 text-center focus:outline-none focus:border-green-500 transition-[border-color] duration-150"
                  />
                </div>
                <button
                  onClick={() => updatePlayer(player.id, { active: false }).then(load)}
                  className="text-xs text-gray-600 hover:text-red-400 active:scale-[0.97] transition-[transform,color] duration-150"
                >
                  {t("players.remove")}
                </button>
              </div>
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              {player.aliases.map((alias) => (
                <span
                  key={alias}
                  className="inline-flex items-center gap-1 text-xs px-2 py-0.5 bg-gray-800 text-gray-400 rounded-full"
                >
                  {alias}
                  <button
                    onClick={() => removeAlias(player, alias)}
                    className="text-gray-600 hover:text-red-400 leading-none transition-colors duration-100"
                  >
                    ×
                  </button>
                </span>
              ...
              <form onSubmit={(e) => addAlias(player, e)} className="flex items-center">
                <input
                  ...
                  className="text-xs w-20 bg-transparent border-b border-gray-700 focus:border-green-500 focus:outline-none text-gray-400 placeholder:text-gray-600 pb-0.5 transition-[border-color] duration-150"
                />
              </form>
            </div>
          </div>
